...
const databaseURL = "https://codeathon-itss.firebaseio.com";
  ...
  if (obj.uid != undefined || obj.uid != "") {
    this.instance
      ...
      .then((value) => {
        ...
        obj.date = Date.now();
        this.db
          .ref("challenges/" + obj.challengeID + "/history/" + obj.uid)
          .set(obj);
        this.db
          .ref("users/" + obj.uid + "/history/" + obj.challengeID)
          .set(obj);
      });
  }
};
...
FirebaseApp.prototype.createTeam = function (ownerID, obj, onFinish) {
  this.db
    .ref("team/")
    .push({ ownerID: ownerID, ...obj, invitation: 0 }, (error) => {
      if (error) {
...
FirebaseApp.prototype.invite = async function (
  teamID,
  teamName,
  ownerName,
  email
  ...
  if (uid != undefined) {
    uid = uid.uid;
    this.db
      .ref("users/" + uid + "/invitation/" + teamID)
      .set({ teamName: teamName, ownerName: ownerName });
  ...
  for (let i = 0; i < historyID.length; i++) {
    if (historyID[i] != "dummy") {
      ...
      if (challenge != undefined) {
        if (
          challenge.canViewSolution == undefined ||
          challenge.canViewSolution == true
        ) {
...
FirebaseApp.prototype.getCompetitionById = async function (competitionId) {
  let data = (
    await this.db.ref("competition/" + competitionId).once("value")
  ).val();
  if (data != undefined) {
    return data;
  }
